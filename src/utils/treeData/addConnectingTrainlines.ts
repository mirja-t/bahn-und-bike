export const addConnectingTrainlines = (
    trainlineStops,
    directTrainStopMap,
    start,
) => {
     ...
     */
    const changeStops = {};
    for (const line in trainlineStops) {
        trainlineStops[line].forEach((s) => {
            const currentStop = directTrainStopMap.retrieve(s.stop_id);
            if (currentStop && s.stop_id !== start) {
                const currentLine = changeStops[s.trainline_id];
                if (currentLine) {
