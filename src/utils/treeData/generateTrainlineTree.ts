...
export const generateTrainlineTree = (
    ...
    const getFirstStop = (startStops) => {
        ...
        return {
            dur: 0,
            trainlines: [
                { id: startStops[0].trainline_id, name: startStops[0].name },
            ],
            pathLength: 0,
    ...
    if (!direct) {
         ...
         *
         */
        const changeStops = addConnectingTrainlines(
            trainlineStops,
            directTrainStopMap,
            startStops[0].stop_id,
        );
        for (const indirectLine in changeStops) {
            const connections = changeStops[indirectLine];
            connections.forEach((change) => {
                const trainstops = [...trainlineStops[indirectLine]];
                const stopNumber = trainstops.find(
                    (s) => s.stop_id === change.stop_id,
                ).stop_number;
                trainlineTree.addIndirectTrainline(
                    change,
                    trainstops,
                    stopNumber,
                );
